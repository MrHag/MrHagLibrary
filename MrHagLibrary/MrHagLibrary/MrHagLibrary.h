	...
	namespace Containers
	{
		...
		public:
			...
			T& operator [](unsigned int a)
			{
				...
				{
					std::cout << a << " - Индекс вышел за пределы памяти ( " << 0 << " - " << len - 1 << " )" << std::endl;
				}
	...
	*/
	...
	namespace StringManipulate
	{
		...
		public:
			...
			void write(std::string* out, const char *pre_str = "", bool save_console_buffer = false)
			{
				std::string buffer;
				int CP = GetConsoleCP();
				SetConsoleCP(1251);

				if (save_console_buffer)
				{
					//SetConsoleOutputCP(1251);
					buffer = cons.Read_console();
				}
		...
		std::string My_Mwrite(const char* text = "", bool save_console_buffer = false)
		{
			Write *Buf = new Write;
			std::string string;
